...
Built-in snippets ship with the Pi setup package. Add personal snippets under
`$PI_CODING_AGENT_DIR/prompt-snippets/` (default: `~/.pi/agent/prompt-snippets/`).
A personal file with the same filename overrides the packaged snippet without
modifying the shared repository.
...
Snippets use one Markdown file each, with frontmatter. Packaged defaults live
in `snippets/` next to `index.ts`; personal overrides live in
`$PI_CODING_AGENT_DIR/prompt-snippets/`.
...
sent, so edits take effect immediately — no `/reload` needed. The package
folder is read-only in some installations; put personal snippets in the user
