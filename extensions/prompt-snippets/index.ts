...
export interface Snippet {
	/** Filename, e.g. "concise.md". User snippets with the same filename override package snippets. */
	id: string;
	name: string;
	description: string;
...
function userSnippetsDir(): string {
	const agentDir = process.env.PI_CODING_AGENT_DIR?.trim() || join(homedir(), ".pi", "agent");
	return join(agentDir, "prompt-snippets");
}
...
/** Load snippets from package defaults and then user overrides, sorted for application. */
export function loadSnippets(sources: SnippetSource[] = [
	{ directory: snippetsDir },
	{ directory: userSnippetsDir() },
]): Snippet[] {
	...
	for (const source of sources) {
		...
		for (const file of readdirSync(source.directory)) {
			...
			try {
				const snippet = parseSnippet(file, readFileSync(join(source.directory, file), "utf8"));
				if (snippet) byId.set(snippet.id, snippet);
			} catch {
				// Skip unreadable or malformed files without disabling the menu.
			}
		}
	}
...
export function composePrompt(text: string, enabledIds: ReadonlySet<string>, available: Snippet[]): string | null {
	const active = sortSnippets(available.filter((snippet) => enabledIds.has(snippet.id)));
...
export default function (pi: ExtensionAPI) {
	...
	async function openMenu(ctx: ExtensionContext) {
		...
		if (snippets.length === 0) {
			ctx.ui.notify(`No snippets found in ${snippetsDir} or ${userSnippetsDir()}`, "warning");
			updateWidget(ctx);
			return;
		}
	...
	pi.on("input", async (event, ctx) => {
		...
		const active = snippets.filter((s) => selected.has(s.id));
		enabled = new Set();
		updateWidget(ctx);

		const text = composePrompt(event.text, selected, active);
		if (text === null) return; // all toggled snippets vanished from disk
		return {
			action: "transform",
			text,
			images: event.images,
