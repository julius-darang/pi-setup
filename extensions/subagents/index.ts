...
const WEB_ACCESS_CANDIDATES = [
	path.join(PI_AGENT_DIR, "npm", "node_modules", "pi-web-access", "index.ts"),
	path.join(PACKAGE_EXTENSIONS_DIR, "..", "npm", "node_modules", "pi-web-access", "index.ts"),
];
...
const CUSTOM_TOOL_EXTENSIONS: Record<string, string> = {
	"web-access": firstExistingPath(WEB_ACCESS_CANDIDATES),
	web_fetch: firstExistingPath([
		path.join(PACKAGE_EXTENSIONS_DIR, "web-fetch", "index.ts"),
		path.join(GLOBAL_EXTENSIONS_DIR, "web-fetch", "index.ts"),
	]),
	safe_bash: path.join(TOOLS_DIR, "safe-bash.ts"),
	video_extract: path.join(GLOBAL_EXTENSIONS_DIR, "video-extract", "index.ts"),
	youtube_search: path.join(GLOBAL_EXTENSIONS_DIR, "youtube-search", "index.ts"),
	google_image_search: path.join(GLOBAL_EXTENSIONS_DIR, "google-image-search", "index.ts"),
...
function loadAgents(): AgentConfig[] {
	...
	for (const entry of fs.readdirSync(AGENTS_DIR)) {
		...
		const subagentAgents = rawSubagentAgents
			...
			: undefined;
		const modelEnvKey = `PI_SUBAGENT_MODEL_${frontmatter.name.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}`;
		agents.push({
...
async function buildPiArgs(
	...
	const nextDepth = Number.isFinite(parentDepth) && parentDepth >= 0 ? Math.floor(parentDepth) + 1 : 1;
