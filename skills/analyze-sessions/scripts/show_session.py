...
def main() -> int:
    ...
    _render(target, args, redact=not args.include_sensitive)
    ...
    if args.include_subagents_content:
        ...
        for sub in subs:
            ...
            print()
            _render(sub, args, redact=not args.include_sensitive)
...
def _render(s: S.SessionSummary, args, *, redact: bool) -> None:
    # Header block
    ...
    for rec in S.iter_records(s.path):
        ...
        ts = _msg_ts(m)
        if role == "user":
            _render_user(m, ts, redact=redact)
        elif role == "assistant":
            _render_assistant(m, ts, args, redact=redact)
        elif role == "toolResult":
            _render_tool_result(m, ts, args.max_tool_output, redact=redact)


def _render_user(m: dict, ts: str, *, redact: bool) -> None:
    text = _join_text(m.get("content"))
    if redact:
        text = S.redact_text(text)
    print(f"## 👤 User  ·  {ts}")
    print()
...
def _render_assistant(m: dict, ts: str, args, *, redact: bool) -> None:
    ...
    for c in m.get("content") or []:
        ...
        if ct == "thinking":
            ...
            print()
            print(S.redact_text(think) if redact else think)
            print()
            print("</details>")
            print()
        elif ct == "text":
            ...
            name = c.get("name") or "?"
            args_str = _summarize_tool_args(c.get("arguments") or c.get("input"), redact=redact)
            print(f"**🔧 {name}**  `{args_str}`")
            print()


def _render_tool_result(m: dict, ts: str, max_chars: int, *, redact: bool) -> None:
...
def _summarize_tool_args(arguments, *, redact: bool) -> str:
    ...
    if isinstance(arguments, dict):
        ...
        for i, (k, v) in enumerate(arguments.items()):
            ...
            if redact:
                v_str = S.redact_text(v_str)
            parts.append(f"{k}={S.truncate(v_str, 80)}")
        return "  ".join(parts)
    value = str(arguments)
    return S.truncate(S.redact_text(value) if redact else value, 120)
