...
def main() -> int:
    ...
    for s in summaries:
        ...
        total_hits += len(hits)
        _print_session_hits(s, hits, redact=not args.include_sensitive)
...
def _print_session_hits(s: S.SessionSummary, hits, *, redact: bool) -> None:
    ...
    for h in hits:
        ...
        snippet = S.redact_text(h["snippet"]) if redact else h["snippet"]
