...
A session is a JSONL file under $PI_CODING_AGENT_DIR/sessions/<encoded-cwd>--/
(or ~/.pi/agent/sessions when the variable is unset), with optional nested
...
def agent_dir() -> Path:
    """Return the Pi directory used for session storage.

    PI_CODING_AGENT_DIR is read at process start so the skill works with the
    same custom agent directory as the Pi installation.
    """
...
def add_privacy_arg(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--include-sensitive",
        action="store_true",
        help="Include unredacted prompt/tool text. Use only for local inspection.",
    )
...
_SECRET_ASSIGNMENT_RE = re.compile(
    r"(?i)(\b(?:api[_ -]?key|access[_ -]?token|auth(?:orization)?|bearer|token|password|passwd|secret|client[_ -]?secret|smtp[_ -]?password|cookie)\b\s*[:=]\s*)(?:\"[^\"]*\"|'[^']*'|[^\s,;}]+)"
)
_BEARER_RE = re.compile(r"(?i)(\bbearer\s+)[A-Za-z0-9._~+/=-]+")
_TOKEN_RE = re.compile(r"\b(?:sk-[A-Za-z0-9_-]{16,}|gh[pousr]_[A-Za-z0-9_]{20,}|AIza[0-9A-Za-z_-]{20,})\b")


def redact_text(text: str) -> str:
    """Redact common credential-shaped values before transcript output."""
