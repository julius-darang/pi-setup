        ...
        for s, prompts in sess_prompts:
            for ts_ms, text in prompts:
                out = {
                    ...
                    "cwd": s.cwd,
                    "timestamp": ts_ms,
                    "text": text if args.include_sensitive else S.redact_text(text),
                }
                print(json.dumps(out, ensure_ascii=False))
        S.stderr(f"# {total_prompts} prompts across {len(sess_prompts)} sessions")
        return 0

    _render_markdown(sess_prompts, total_prompts, redact=not args.include_sensitive)
    return 0
...
def _render_markdown(sess_prompts, total_prompts: int, *, redact: bool) -> None:
    ...
    for proj in proj_order:
        ...
        for s, prompts in sorted(by_proj[proj],
            ...
            for _, text in prompts:
                ...
                output_text = S.redact_text(text) if redact else text
                for line in output_text.splitlines() or [""]:
                    print(f"> {line}")
                print()
